...
link1="https://" #Ссылка на Софт
link2="https://"
password="смотрите в инструкции" #Пароль от архива с Софтом
ukey = '' #Индивидуальный ключ активации Софта
...
try:
    ...
    pass
    # users_q.write('0') #если файл создался только что, мы начинаем отчет пользователей с 0, т.е. в начале файла пишем количество пользователей, написавших команду >get_soft
...
async def hlps(ctx):
    ...
    embed.add_field(name='$info',value='Информация о Софте', inline=False)
...
@client.command(aliases = ['get_soft']) #функция выдачи Софта
async def gc(ctx):
    global num_of_users
    global ukey
    ...
    await ctx.send("Софт отправлен в личные сообщения!") #отчет об отправке
...
@client.command(aliases = ['info']) #Функция вывода рекламы Софта
async def inf(ctx):
    embed = discord.Embed(colour=ctx.author.color,timestamp=ctx.message.created_at)
    embed.add_field(name="Информация о Софте", value="Этот софт создан для помощи", inline=False)
    embed.add_field(name="Описание", value="Новый бесплатный и рабочий софт.", inline=False)
    await ctx.author.send(embed=embed)
    await ctx.send("Информация отправлена в личные сообщения!") #отчет об отправке
    logwrite(ctx, 'get info about soft')

@client.command(aliases = ['sar']) #Функция отправки рекламы в лс всем, кто на сервере
async def members(ctx):
    embed = discord.Embed(colour=ctx.author.color,timestamp=ctx.message.created_at)
    embed.add_field(name="Обновление! (version 5.4.16)", value="Вышла наиболее актуальная версия Софта! Для получения ссылки на загрузку, зайдите в наш канал и напишите $get_soft", inline=False)
    for Every_Member in ctx.guild.members:
        if Every_Member.bot != True:
            await Every_Member.send(embed=embed)
    logwrite(ctx, 'send a add')

# @client.command(aliases = ['Отправь_правила']) #Функция вывода рекламы Софта
# async def rules(ctx):
#     embed = discord.Embed(colour=ctx.author.color,timestamp=ctx.message.created_at)
...
# @client.command(aliases = ['У_нас_новость']) #Функция вывода рекламы Софта
# async def news(ctx):
